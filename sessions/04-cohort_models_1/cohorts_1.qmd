...
-   No interaction between individuals (e.g., infection)
...
-   Individuals in the same state treated **identically**
    ...
    -   same probability of transitioning out
...
-   A Markov property 'workaround': If costs, outcomes, or transitions differ based on time in health state, break out 1+ tunnel states
:::
:::::
...
Therefore (full Markov model matrix solution):
...
# disc_mults <- 1/((1+disc_rate)^((1:n_cycles)*cycle_length))
disc_mults
...
disc_mults <- 1/((1+(disc_rate*cycle_length))^(1:n_cycles))
rowSums(m_cost_by_cyclestate[1:7,]) #undiscounted cost by year
# WITH DICOUNTING
rowSums(m_cost_by_cyclestate[1:7,])*disc_mults #Discounted costs by year
