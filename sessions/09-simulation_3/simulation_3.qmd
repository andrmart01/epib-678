---
title: "Efficient simulation & ABM"
...
-   [**Efficient coding (general)**]{.underline}
...
-   Using `sum` over a comparison (with \<, \>, \<=, or \>=) is more efficient than for loop + if statement

-   `runif`, `^`, `+` each called 2x on a vector, not 2N times on a single number
...
N = 5000000 # 5 Million simulation runs
A <- monte_carlo(N)
B <- monte_carlo_vec(N)
...
-   [**Efficiency techniques for simulation**]{.underline}
...
## Vectorization in individual simulation model
...
-   Generate baseline characteristics for everyone at once
    ...
    -   Generate random variables en masse with `runif(N)` not `runif(1)` (or `rgamma`, `rbeta`, etc.)

## Vectorization in individual simulation model

-   Where possible, avoid loops and use **linear algebra** to compute calculations for the whole cohort (or large subsets) at once

    -   Multiplying, adding, subtracting, exponentiating matrices and vectors

-   In discrete time models, transition all individuals through each cycle simultaneously
...
-   **Efficient coding** lets you simulate N individuals faster

-   **Variance reduction** to lets you simulate smaller N with less uncertainty in outcomes of interest
...
-   Otherwise, variance in baseline population characteristics will contribute to variance in outcomes between strategies
...
Generate two **negatively-correlated sets** of random variables: U, original set, and V the antithetic set.
...
# Calculation on original variables
x_1<- unif_vec < 0.3

# Calculation on antithetic variables
x_2<- (1-unif_vec) < 0.3

h_or_t_antithetic <- (x_1 + x_2)/2
mu=mean(h_or_t_antithetic)
SE=1.96*sd(h_or_t_antithetic)/sqrt(n/2)
print(c(Mean=mu,SE=SE,Lower=mu-SE,Upper=mu+SE))
```
...
-   When modeling \>1 policy, using the same random variables in the same place within model reduces variance in estimated ***incremental*** outcomes ($\Delta$costs, $\Delta$QALYs).
...
# Sample n random variables from each stream
rstream.sample(stream1, n)
rstream.sample(stream2, n)

# Can sample antithetic variates
rstream.antithetic(stream1) <- TRUE
```

Source: [Rstream manuscript](https://www.iro.umontreal.ca/~lecuyer/myftp/papers/rstream.pdf)
...
-   Simulate same cohort in all strategies, common random numbers (if you can easily align them)

**Consider others** if needed

-   Antithetic variates, conditional Monte Carlo (if rare events are an issue)

## Recap on efficient simulation
...
-   9 of 16 methods lectures completed

-   Remaining method lectures cover:

    -   One more model type (differential equations)

    -   Methods for model design, parameterization, calibration, and uncertainty analysis
...
-   **Assignment 3: Simulation** due Mon, Feb 16
    -   Part 1 (simulated decision tree) available now
    -   Part 2 (DES) will be released by Thursday
